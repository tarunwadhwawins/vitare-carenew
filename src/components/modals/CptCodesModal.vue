  ...
  <a-modal max-width="1140px" width="100%" title="Add CPT Codes" centered :footer="null" :maskClosable="false" @cancel="closeModal()" >
    ...
    <a-row :gutter="24">
      <a-col :sm="12" :xs="24">
            ...
            <a-form-item :label="$t('global.service')" name="serviceId"
              :rules="[{ required: true, message: $t('global.service') +' '+$t('global.service')+' '+$t('global.validation')  }]">
              <a-select ref="select" v-if="cptCodesGetters.service" v-model:value="cptCodeForm.serviceId" style="width: 100%"
                size="large">
      ...
      <a-col :sm="6" :xs="24">
         <div class="form-group">
                    <a-form-item label="Billing Amount" name="billingAmout" :rules="[{required: true,message:'Billing Amount ' +$t('global.validation'),},]">
                       <a-input-number
    v-model:value="cptCodeForm.billingAmout"
      ...
      <a-col :sm="6" :xs="24">
          <div class="form-group">
            <a-form-item :label="$t('global.duration') +' '+$t('global.time')" name="durationId"
              ...
              <a-select ref="select" v-if="durationList.duration" v-model:value="cptCodeForm.durationId" style="width: 100%"
                size="large">
                <a-select-option value="" hidden>{{'Select Duration Time'}}</a-select-option>
                <a-select-option v-for="duration in durationList.duration['globalCode']" :key="duration.id" :value="duration.id">
...
store.dispatch('cptCodesList')
    ...
    function closeModal() {
            if (cptCodeForm.name != "" || cptCodeForm.description != "" || cptCodeForm.serviceId != "" || cptCodeForm.billingAmout != "" ||cptCodeForm.durationId != "") {
