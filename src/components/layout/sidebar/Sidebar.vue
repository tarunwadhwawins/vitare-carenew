...
<template>
<a-layout-sider v-show="accessPermission!=0" :style="{ overflow: 'auto', height: '100vh', position: 'fixed', left: 0 }">
    <div class="menuList">
        <a-menu>
            ...
            <router-link to="/manage-care-coordinator" v-if="arrayToObjact(patientsPermissions, 40)">
                <a-menu-item>
                    <UserOutlined /><span class="menuItem">{{$t('sidebar.careCoordinator')}}</span></a-menu-item>
            </router-link>
            <router-link to="/manage-patients" v-if="arrayToObjact(patientsPermissions, 65)">
                <a-menu-item>
            ...
            <router-link to="/cpt-codes" v-if="roles==1">
                <a-menu-item>
